...
class CtrlGenerador extends CControlador{
    ...
    public function antesDeIniciar() {
        ...
        if($accion != 'login' && !Sis::Sesion()->existeAtributo("codegen_log")){
    ...
    public function accionLogin(){
        ...
        if(isset($this->_p['log'])) {
            $usr = Sis::apl()->modulo->usuario;
            $clv = Sis::apl()->modulo->clave;
            if($usr == $this->_p['log']['username'] && $clv == $this->_p['log']['password']){
                Sis::Sesion()->setAtributo("codegen_log", $this->_p['log']['username']);
    ...
    public function accionLogout(){        
        if(Sis::Sesion()->existeAtributo("codegen_log")){
            Sis::Sesion()->borrarAtributo("codegen_log");
        }
        $this->redireccionar('login');
    }
    ...
    private function generarModelo(){
        ...
        $error = false;
        if(!$sobreescribir && file_exists(Sis::resolverRuta("!aplicacion.modelos"). DS . "$nombre.php")){
            $msg = "El modelo ya existe";
        } else {
            $modelo = $this->generador->generarModelo($this->_p['tabla'], $this->_p['nombre']);
            if(!$modelo){
        ...
        Sis::Sesion()->setNotificacion("modelo", ['error' => $error, 'msg' => $msg]);
    }
    ...
    public function accionCrud(){
        ...
        if(isset($this->_p['crear-crud'])){
            ...
            $crud = $this->generador->generarCrud($this->_p['tabla'], $this->_p['nombre'], $archivos, $plantilla, "Jorge Alejandro Quiroz Serna <alejo.jko@gmail.com>");
            ...
            Sis::Sesion()->setNotificacion("crud", [
                'error' => $error,
                'msg' => $msg,
            ]);
    ...
    private function validarCrud(){
        $t = str_replace($this->esquema->getPrefijo(), '', $this->_p['tabla']);
        $nombre = str_replace(' ', '', ucwords(str_replace('_', ' ', $t)));
        $modelo = file_exists(Sis::resolverRuta("!aplicacion.modelos") . DS . "$nombre.php");
        $controlador = file_exists(Sis::resolverRuta("!aplicacion.controladores") . DS . "Ctrl$nombre.php");
        $vistas = is_dir(Sis::resolverRuta("!aplicacion.vistas." . lcfirst($nombre)));
    ...
    public function accionModulo(){
        ...
        if(isset($this->_p['crear-mod'])){
            ...
            if(!$modulo["error"]){
                ...
                Sis::Sesion()->setNotificacion("modErr", 'Ocurrió un error');
            } else {           
                $codigo = "\t'" . lcfirst($modulo['clase']) . "' => [\n"
                        ...
                        . "\t],";
                Sis::Sesion()->setNotificacion("modCreado", [
    ...
    private function importarEsquema(){
        $importacion = Sis::importar('!web.modulos.codegen.clases.CEsquema');
        if(!$importacion){
    ...
    private function importarGenerador(){
        $importacion = Sis::importar('!web.modulos.codegen.clases.CGenerador');
        if(!$importacion){
