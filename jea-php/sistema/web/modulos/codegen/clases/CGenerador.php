...
class CGenerador {
     ...
     * @return boolean
     */
    public function generarModelo($tabla, $nombre = "", $plantilla = 'basico', $paraCrud = false){
        ...
        $rutaPlantilla = !$paraCrud? 
                Sis::resolverRuta("!web.modulos.codegen.plantillas.modelo.$plantilla", true) : 
                Sis::resolverRuta("!web.modulos.codegen.plantillas.crud.$plantilla.modelo", true);
        ...
        $rutaModelos = Sis::resolverRuta("!aplicacion.modelos");        
    ...
    public function generarModulo($nombre, $plantilla = "basico"){
        ...
        $ruta = Sis::resolverRuta("!modulos.$nCarpeta");
        # creamos la carpeta si no existe
        if(!is_dir($ruta)){ mkdir($ruta); }
        
        # creamos la ruta de las plantillas
        $rPlantilla = Sis::resolverRuta("!web.modulos.codegen.plantillas.modulo.$plantilla");
    ...
    public function obtenerArchivosCrud($plantilla = "basica"){
        $rutaPlantillas = Sis::resolverRuta("!web.modulos.codegen.plantillas.crud.$plantilla");
     ...
     * @return boolean
     */
    public function generarCrud($tabla, $nombre, $archivos = [], $plantilla = 'basica', $autor = ''){
        $rutaPlantilla = Sis::resolverRuta("!web.modulos.codegen.plantillas.crud.$plantilla");
        $proyectoDes = Sis::resolverRuta("!aplicacion");
        # generamos el controlador en caso de que exista
        $modelo = $this->generarModeloCrud($archivos, $tabla, $nombre, $plantilla);        
        ...
        $ctrl = $this->generarControlador($nTabla, $nombre, $archivos, $rutaPlantilla, $proyectoDes.DS.'controladores', $autor);
        
        $carpetaCtrl = lcfirst($nombre);
        ...
        $vistas = $this->generarVistas($nTabla, $nombre, $archivos, $rutaPlantilla, $desVistas);
     ...
     * @return boolean
     */
    private function generarModeloCrud(&$archivos, $tabla, $nombre, $plantilla){
        $indiceModelo = array_search('modelo.php', $archivos);
        if($indiceModelo === false){ return true; }
        unset($archivos[$indiceModelo]);
        return $this->generarModelo($tabla, $nombre, $plantilla, true);
    }
     ...
     */
    private function generarControlador($tabla, $nmodelo, &$archivos, $rutaPlantilla, $rutaDes, $autor){
        ...
        return $this->guardarArchivo($rutaDes, "Ctrl" . $nmodelo, $strPlantilla);
    }
     ...
     * @return boolean
     */
    private function generarVistas($tabla, $nmodelo, $archivos, $rutaPlantilla, $rutaDestino){
        Sis::importar("!aplicacion.modelos.$nmodelo");
        $modelo = new $nmodelo();
        $error = false;
        ...
        foreach ($archivos AS $archivo){
            ...
            $contenido = $this->cargarPlantilla($ruta, ['nTabla' => $tabla, 'nModelo' => $nmodelo, 'modelo' => $modelo]);
