...
//            throw new CExAplicacion("No se ha cerrado apropiadamente el formualrio, recuerde usar \$formulario->cerrar())");
    ...
    /**
     * Esta función crea un campo lista de selección pasando solo el nombre del modelo y los atributos
     * que se mostrarán
     * @param CModelo $modelo
     * @param string $atributo
     * @param array $elementos
     ...
     * @return string
     */    
    public function listaM($modelo = null, $atributo = '', $tabla = null, $id = '', $texto = '', $opciones = []){
        $modelos = call_user_func([$tabla, 'modelo'])->listar();
        $elementos = CHtml::modeloLista($modelos, $id, $texto);
        return $this->lista($modelo, $atributo, $elementos, $opciones);
    }
