...
final class CHtml {
    ...
    public static function link($texto = '', $url = [], $opciones = []){
        $opciones['href'] = Sis::apl()->crearUrl($url);
    ...
    public static function codificar($cadena){
        return htmlentities($cadena, ENT_QUOTES, Sis::apl()->charset);
    }
    ...
    public static function modeloLista($modelos, $valor, $texto){
        # si el modelo ingresado es string se procederá a listar
        if(is_string($modelos)){
            $n = $modelos;
            $modelos = call_user_func([$n, 'modelo'])->listar();
        }
        $elementos = [];
