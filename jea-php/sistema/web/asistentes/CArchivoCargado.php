...
class CArchivoCargado {
    ...
    private function __construct() {
        $this->destinoPorDefecto = Sis::resolverRuta('!raiz.publico.archivos');
    }
    ...
    public static function instanciarPorNombre($nombre = ''){
        if(isset($_FILES[$nombre]) && is_array($_FILES[$nombre])){
            ...
            $instancia->nombre = self::extraerNombre($archivo['name']);
            $instancia->nombreOriginal = $archivo['name'];
    ...
    /**
     * Esta función permite obtener una instancia del archivo usando el nombre del modelo
     * y el campo que el archivo representa en este.
     * @param string $modelo
     * @param string $campo
     * @return CArchivoCargado
     */
    public static function instanciarModelo($modelo, $campo){
        if(isset($_FILES[$modelo]) && is_array($_FILES[$modelo])){
            $archivo = $_FILES[$modelo];
            $instancia = new CArchivoCargado();
            $instancia->nombre = self::extraerNombre($archivo['name'][$campo]);
            $instancia->nombreOriginal = $archivo['name'][$campo];
            $instancia->tipo = $archivo['type'][$campo];
            $instancia->ubicacion = $archivo['tmp_name'][$campo];
            $instancia->error = $archivo['error'][$campo];
            $instancia->peso = $archivo['size'][$campo];
            return $instancia;
        }
    }
    ...
    public static function instanciarTodasPorNombre($nombre = ''){
        if(isset($_FILES[$nombre]) && is_array($_FILES[$nombre])){
            ...
            foreach($_FILES[$nombre]['name'] AS $clave=>$valor){
                $instancia = new CArchivoCargado();
                $instancia->nombre = self::extraerNombre($valor);
                $instancia->nombreOriginal = $valor;
    ...
    /**
     * Esta función ayuda a extraer el nombre
     * @param string $nombre
     * @return string
     */
    private static function extraerNombre($nombre){
        return substr($nombre, 0, strrpos($nombre, '.'));
    }
     ...
     * @param string $rutaDestino
     * @param array $config pre, tamanio, x, y, tipo, calidad, autocentrar = false
     * @return boolean
     */
    public function thumbnail($rutaDestino = "", $config = []){
        ...
        $imagenGenerada = $this->guardarThumb($thumb, $destino . DS . $pre.$nGuardado, $tipo, $calidad);
        imagedestroy($thumb);
        imagedestroy($imgOrg);
    ...
    public function getExtension(){
        $partes = explode('.', $this->nombreOriginal);
        return end($partes);
    }
