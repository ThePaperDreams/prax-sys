...
final class CBoot {
    ...
    public static function input($tipo = 'text', $valor = '', $opciones = []){
        ...
        if(isset($opciones['group'])){
            $label = isset($opciones['label'])? 
                CHtml::e('label', $opciones['label'], ['for' => isset($opciones['id'])? $opciones['id'] : '']) :
                    "";
            unset($opciones['group']);
    ...
    /**
     * Esta función permite crear un campo númerico con estilos de bootstrap
     * @param string $valor
     * @param array $opciones
     * @return string
     */
    public static function number($valor = '', $opciones = []){
        return self::input('number', $valor, $opciones);
    }
    ...
    private static function inputAddOn($valor = '', $tipo = 'text', $opciones = []){
        ...
        return $group? 
            CHtml::e('div', $label . $html, ['class' => 'form-group']) :
            $label . $html;
    }
    ...
    public static function select($seleccion = '', $elementos = [], $opciones = []){
        ...
        $label = isset($opciones['label'])? 
            CHtml::e('label', $opciones['label'], ['for' => isset($opciones['id'])? $opciones['id'] : '']) :
                "";
        $lista = $label . CHtml::lista($seleccion, $elementos, $opciones);
        
        # si hay que agrupar
        if(isset($opciones['group']) && $opciones['group'] == true){
            unset($opciones['group']);
            return CHtml::e('div', $lista,['class' => 'form-group']);
        }
    ...
    private static function listGroupBase($elementos = [], $opciones = [], $c = 'div', $i = 'div'){
        ...
        foreach ($elementos AS $elemento){
            ...
            if(isset($opcE['url'])){
                $opcE['href'] = Sis::apl()->crearUrl($opcE['url']);
                unset($opcE['url']);
            }
