...
abstract class CControlador extends CComponenteAplicacion{
    ...
    protected $_g;
    /**
     * Este array almacenará las migas de pan que debe tener cada página
     * @var array 
     */
    protected $migas = [];
    /**
     * Este array almacena las opciones que estarán disponibles ne cada página
     * @var array 
     */
    protected $opciones = [];
    ...
    public function __construct($ID, $accion) {
        ...
        $this->rutaVistas = Sis::resolverRuta('!aplicacion.vistas');
    ...
    public function redireccionar($ruta, $parametros = []){
        ...
        if(count($partes) === 1 && !$this->perteneceAModulo){
            ...
            $ruta = Sis::apl()->modulo->ID."/".$this->ID.'/'.$ruta;
        }
        $url = Sis::apl()->crearUrl(array_merge(array($ruta), $parametros));
        header("location:".$url);
    ...
    /**
     *  Esta función permite redireccionar sin instanciar el controlador
     * @param string $controlador
     * @param string $accion
     * @param array $parametros
     */
    public static function redireccionarA($controlador, $accion, $parametros = []){
        $url = Sis::apl()->crearUrl(array_merge(["$controlador/$accion"], $parametros));
        header("location:".$url);
        # tenemos que finalizar la ejecución de la aplicación
        Sis::fin();
    }
    ...
    public function mostrarVista($vista = '', $parametros = []){
        ...
        Sis::Recursos()->incluirRecursos($this->contenido);
        echo $this->contenido;
    }
 
    /**
     * esta función es un alias de mostrarvista
     * @param string $vista
     * @param array $parametros
     */
    public function vista($vista, $parametros = []){
        $this->mostrarVista($vista, $parametros);
    }
    ...
    /**
     * Esta función es un alias de mostrarVistaP
     * @param string $vista
     * @param array $parametros
     * @return string
     */
    public function vistaP($vista, $parametros = []){
        return $this->mostrarVistaP($vista, $parametros);
    }
    ...
    private function validarVista($vista, $plantilla = false){
        ...
        $rutaVistaTema = Sis::apl()->tema !== null? 
                (Sis::apl()->tema->getRutaBase().DS."vistas".DS."$vista.php") : "";
        $buscarEnTemas = Sis::apl()->tema !== null && file_exists($rutaVistaTema);
        ...
        if((!$this->perteneceAModulo && $buscarEnTemas) || 
            ...
            $rutaVista = Sis::apl()->tema->getRutaBase().DS."vistas";
        } else {
    ...
    /**
     * Esta función limpia el contenido del controlador
     */
