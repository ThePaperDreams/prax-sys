...
abstract class CComponenteUsuario extends CComponenteAplicacion {
    ...
    public function antesDeIniciar() {
        if(Sis::Sesion()->existeAtributo($this->_identificador)){
            $sesion = Sis::Sesion()->getAtributo($this->_identificador);
    ...
    public function iniciarSesion($id, $usuario){
        ...
        Sis::Sesion()->setAtributo($this->_identificador, [
            'id' => $id,
            'usuario' => $usuario,
        ]);
        $this->esVisitante = false;
    ...
    private function ponerAtributosEnSesion(){
        ...
        Sis::apl()->mSesion->setAtributo($this->_identificador, [
            'id' => $this->id,
    ...
    public function cerrarSesion(){
        ...
        Sis::Sesion()->borrarAtributo($this->_identificador);
    ...
    /**
     * Esta función se encarga de asignar los atributos que hayan sido configurados para
     * el componenters de Usuario
     */
    public function cargarConfiguracion(){
        $config = Sis::apl()->getComponente('usuario');
        $this->asignarAtributos($config);
    }
}