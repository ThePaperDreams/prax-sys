...
class CBDComponente extends CComponenteAplicacion{
    ...
    public function ejecutarComando($comando, $comoObjeto = false){
    ...
    public function __call($nombre, $argumentos) {        
        if(method_exists($this->controlador, $nombre)){
            return call_user_func_array([$this->controlador, $nombre], $argumentos);
        }
    }
