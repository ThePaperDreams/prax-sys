...
abstract class CBaseModelo {
    ...
    public function __construct() {        
        ...
        $this->_alias = Sis::apl()->bd->controlador->alias;
    }
    ...
    protected function _insertar(){
        ...
        if($resultado){
            $this->_atributos[$this->_pk] = Sis::apl()->bd->controlador->ultimoId();
            $this->_nuevo = false;
        }
        return $resultado;
    }
    ...
    private function ejecutar($tipo = self::_CONSULTA_){
        switch ($tipo){
            case self::_CONSULTA_: 
                return Sis::apl()->bd->controlador->consultar();
            case self::_INSERCION_: 
                return Sis::apl()->bd->controlador->insertar();                
            case self::_ACTUALIZACION_ : 
                return Sis::apl()->bd->controlador->actualizar();
            case self::_ELIMINACION_:
                return Sis::apl()->bd->controlador->eliminar();
            default : 
